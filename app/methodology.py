...
def render_methodology():
    ...
    st.markdown("---")
    st.markdown("<h3 style='color:#F8FAFC;'>Multi-Agent Guardrails</h3>", unsafe_allow_html=True)
    st.markdown(
        "- Agents are deterministic decision-support modules; they do not invent facts.\n"
        "- All factual claims must reference source files or documented rules.\n"
        "- Scoring is reproducible and numeric outputs are produced by deterministic code paths.\n"
        "- LLM-style narrative is permitted only for human-readable explanations, never for changing scores.\n"
        "- Records marked blocked for compliance must never be recommended for outreach."
    )
