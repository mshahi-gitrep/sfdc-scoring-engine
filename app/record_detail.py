...
def render_record_detail():
    ...
    st.markdown(
        ...
        "<p style='color:#64748B; margin:0; font-size:1.05rem;'>A complete briefing document that explains exactly why this prospect is ready and what to say.</p>"
        "</div>",
    ...
    st.markdown(
        ...
        f"<div style='flex:1;'>"
        f"<h2 style='margin:0 0 4px; color:#1E293B; font-size:1.8rem;'>{row['full_name']}</h2>"
        f"<p style='color:#64748B; margin:0; font-size:1rem;'>{row['title'] or 'Title not provided'}</p>"
        f"<p style='color:#3B82F6; margin:8px 0 0; font-weight:600;'>{company_name}</p>"
        f"</div>"
        f"<div style='text-align:right;'>"
        f"<div style='background:rgba(22, 163, 74, 0.1); color:#16A34A; padding:10px 16px; border-radius:10px; font-weight:700; margin-bottom:8px; font-size:1.1rem;'>Readiness {row['readiness_score']:.0f} / 100</div>"
    ...
    st.markdown(
        "<div style='padding:24px; border-radius:16px; background:#F8FAFC; border:1px solid rgba(22, 163, 74, 0.15); margin-bottom:28px;'>"
        f"<h3 style='margin:0 0 12px; color:#16A34A; font-size:1.2rem;'>✓ Recommended Action</h3>"
        f"<p style='margin:0; color:#1E293B; font-size:1.05rem; font-weight:600;'>{action_text}</p>"
        "</div>",
        unsafe_allow_html=True
    )
    ...
    with col_pos:
        st.markdown(
            "<div style='padding:20px; border-radius:14px; background:#F8FAFC; border:1px solid rgba(22, 163, 74, 0.15);'>"
            "<h4 style='color:#16A34A; margin:0 0 12px;'>✔️ Positive Signals</h4>",
            unsafe_allow_html=True
        )
        if positives:
            ...
            st.markdown("<p style='color:#64748B;'>No clear signals</p>", unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)

    with col_neg:
        st.markdown(
            "<div style='padding:20px; border-radius:14px; background:#F8FAFC; border:1px solid rgba(245,158,11,0.2);'>"
            "<h4 style='color:#EA8C1C; margin:0 0 12px;'>⚠️ Concerns</h4>",
            unsafe_allow_html=True
        )
        if negatives:
            ...
            st.markdown("<p style='color:#64748B;'>No major blockers</p>", unsafe_allow_html=True)
        st.markdown("</div>", unsafe_allow_html=True)
    ...
    if row.get("agentic_recommendation_available"):
        st.markdown(
            "<div style='padding:24px; border-radius:16px; background:#F8FAFC; border:1px solid rgba(59,130,246,0.2); margin-bottom:28px;'>"
    ...
    if not timeline_df.empty:
        st.markdown(
            "<h3 style='color:#1E293B; margin-bottom:16px; font-size:1.2rem;'>📅 Recent Engagement Timeline</h3>",
            unsafe_allow_html=True
        )
        ...
        st.markdown(
            "<div style='padding:16px; border-radius:12px; background:#F8FAFC; border:1px solid rgba(0,0,0,0.08);'>"
            "<p style='color:#64748B; margin:0;'>No campaign activity yet for this prospect</p>"
            "</div>",
            unsafe_allow_html=True
        )
    ...
    st.markdown("")
    st.markdown(
        "<div style='padding:24px; border-radius:16px; background:linear-gradient(135deg, rgba(22, 163, 74, 0.08) 0%, rgba(34,197,94,0.05) 100%); border:1px solid rgba(22, 163, 74, 0.15);'>"
        "<h3 style='color:#16A34A; margin:0 0 12px; font-size:1.1rem;'>✓ Ready to Reach Out</h3>"
        f"<p style='margin:0; color:#64748B; line-height:1.6;'>"
